...
const styles: Partial<Record<TournamentStatus, string>> = {
...
export function TournamentStatusBadge({ status }: { status: Exclude<TournamentStatus, "DRAFT" | "CANCELLED"> }) {
  return (
    <span className={`inline-flex shrink-0 rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${styles[status]}`}>
      {status.replaceAll("_", " ")}
    </span>
  );
}
