...
type RoundHistoryItem = { id: string; crashMultiplier: string | number | null; crashedAt: string | null };
...
export function AviatorGame() {
  ...
  const loadHistory = async () => {
    try {
      const [betResponse, roundResponse] = await Promise.all([
      ...
      if (roundResponse.ok) setRoundHistory((await roundResponse.json()) as RoundHistoryItem[]);
    } catch {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (payload.snapshot) {
            ...
            void loadBets(payload.snapshot.roundId);
          }
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-lime-300">Games</p><h1 className="mt-2 text-3xl font-black text-white">Aviator</h1></div>
        <div className="flex items-center gap-3">{walletBalance !== null ? <span className="rounded-full border border-lime-300/20 bg-lime-300/5 px-3 py-1 text-xs font-bold text-lime-200">Balance ₹{walletBalance}</span> : null}<span className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-300">{connection}</span></div>
      </div>

      <section className="grid gap-6 lg:grid-cols-[1.6fr_0.9fr]">
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/70 p-6 shadow-2xl sm:p-10"><div className={`flex min-h-[360px] flex-col items-center justify-center rounded-2xl border bg-white/[0.025] text-center ${snapshot.phase === "CRASHED" ? "border-red-400/30" : "border-white/5"}`}><p className="text-sm font-semibold text-slate-400">{status}</p>{countdown ? <p className="mt-3 text-lg font-bold text-slate-300">Starting in {countdown}s</p> : null}<div className={`my-5 text-7xl font-black tracking-tight tabular-nums sm:text-8xl ${snapshot.phase === "CRASHED" ? "text-red-300" : "text-white"}`}>{snapshot.multiplier.toFixed(2)}x</div><p className="text-xs text-slate-500">Round {snapshot.roundId}</p></div></div>
        ...
        <aside className="rounded-3xl border border-white/10 bg-white/[0.035] p-6">
          ...
          {authenticated === false ? <div className="mt-5 rounded-2xl border border-white/10 bg-slate-950/50 p-4 text-sm text-slate-300"><p>Sign in to use the wallet and place bets.</p><Link href="/login" className="mt-4 inline-flex rounded-xl bg-lime-300 px-4 py-2 font-bold text-slate-950">Sign in with Google</Link></div> : <>
            <label className="mt-5 block text-sm font-semibold text-slate-300" htmlFor="aviator-amount">Bet amount</label>
            <div className="mt-2 flex items-center gap-2"><button type="button" onClick={() => setAmount((value) => Math.max(1, Number(value) - 10).toFixed(2))} className="size-11 rounded-xl border border-white/10 text-xl text-white">−</button><div className="flex flex-1 items-center rounded-xl border border-white/10 bg-slate-950 px-3"><span className="text-slate-500">₹</span><input id="aviator-amount" value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" className="w-full bg-transparent px-2 py-3 text-center font-bold text-white outline-none" /></div><button type="button" onClick={() => setAmount((value) => (Number(value) + 10).toFixed(2))} className="size-11 rounded-xl border border-white/10 text-xl text-white">+</button></div>
            <div className="mt-5 flex items-center justify-between"><span className="text-sm font-semibold text-slate-300">Auto Cashout</span><button type="button" onClick={() => setAutoEnabled((value) => !value)} className={`rounded-full px-3 py-1 text-xs font-bold ${autoEnabled ? "bg-lime-300 text-slate-950" : "border border-white/10 text-slate-400"}`}>{autoEnabled ? "ON" : "OFF"}</button></div>
