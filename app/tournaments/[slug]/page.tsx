...
async function getPublicTournament(slug: string) {
  return prisma.tournament.findFirst({
    where: { slug, status: { in: [...PUBLIC_STATUSES] }, game: { isActive: true } },
    select: {
      id: true,
      name: true,
      slug: true,
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!tournament) return { title: "Tournament Not Found | PlayerLobby", description: "The requested public tournament could not be found." };
  ...
  return {
    ...
    openGraph: { title: `${tournament.name} | PlayerLobby`, description: description.slice(0, 160), ...(bannerUrl ? { images: [bannerUrl] } : {}) },
  };
}
...
export default async function TournamentDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const registrationEnd = tournament.registrationEndTime ? formatAppDateTime(tournament.registrationEndTime) : "Not announced";
  const loginHref = `/login?callbackUrl=${encodeURIComponent(`/tournaments/${slug}`)}`;

  return (
    <main>
      <SectionContainer className="py-8 sm:py-12">
        <Button href="/tournaments" variant="secondary" className="mb-5">← Back to Tournaments</Button>
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025] shadow-2xl shadow-black/20">
          <div className="relative min-h-[300px] overflow-hidden bg-slate-950 sm:min-h-[380px]">
            {bannerUrl ? (
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${JSON.stringify(bannerUrl)})` }} aria-hidden="true" />
            ) : (
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(163,230,53,0.18),transparent_35%),linear-gradient(135deg,#0f172a,#020617)]" aria-hidden="true" />
            )}
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(300px,1fr)]">
          <div className="space-y-6">
            <section className="rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:p-8">
              ...
              <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-400 sm:text-base">{tournament.description?.trim() || "No description provided."}</p>
            </section>
            <section className="rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:p-8">
              <h2 className="text-xl font-black text-white">Rules</h2>
              <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-400 sm:text-base">{tournament.rules?.trim() || "Rules will be announced soon."}</p>
            </section>
          </div>
          ...
          <aside className="space-y-6">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:p-7">
              ...
              <p className="mt-5 rounded-2xl border border-white/10 bg-slate-950/40 p-4 text-xs leading-5 text-slate-500">Tournament joining, room credentials, and other participation details are handled in later platform phases.</p>
            </section>
          </aside>
        </div>
      </SectionContainer>
