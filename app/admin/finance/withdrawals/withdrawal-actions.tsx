...
export function WithdrawalActions({ withdrawalId, status, payoutId, payoutStatus, destinationType }: { withdrawalId: string; status: string; payoutId?: string; payoutStatus?: string; destinationType?: "UPI" | "BANK_ACCOUNT" }) {
  ...
  if (status === "PENDING") return <div className="space-y-3">
    <form action={approve} onSubmit={(event) => { if (!window.confirm("Approve this withdrawal request? This does not initiate a PayU transfer.")) event.preventDefault(); }}>
  ...
  if (status === "PAYOUT_INITIATED" || status === "PROCESSING") return <div className="space-y-2">
    {payoutId ? <form action={check}><input type="hidden" name="payoutId" value={payoutId} /><button type="submit" disabled={checking} className="w-full rounded-lg border border-amber-300/20 px-3 py-2 text-xs font-bold text-amber-200 disabled:opacity-50">{checking ? "Checking PayU…" : "Check PayU Status"}</button></form> : null}
    <p className="text-[11px] text-slate-500">No retry is allowed while the transfer is uncertain or processing.</p>
