...
export function WithdrawalActions({ withdrawalId, status, payoutId, destinationType }: Props) {
  ...
  const defaultType = destinationType === "UPI" ? "UPI" : "IMPS";
  ...
  if (status === "APPROVED") return <div className="space-y-2">
    <form action={process}><input type="hidden" name="withdrawalId" value={withdrawalId} /><label className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Payment type</label><select name="paymentType" defaultValue={defaultType} className="mt-1 w-full rounded-lg border border-white/10 bg-slate-950 px-2 py-2 text-xs text-white">{destinationType === "UPI" ? <option value="UPI">UPI</option> : <><option value="IMPS">IMPS</option><option value="NEFT">NEFT</option><option value="RTGS">RTGS</option></>}</select><button type="submit" disabled={processing} className="w-full rounded-lg bg-lime-300 px-3 py-2 text-xs font-black text-slate-950 disabled:opacity-50">{processing ? "Sending to PayU…" : "Process payout"}</button></form>
  ...
  if ((status === "PAYOUT_INITIATED" || status === "PROCESSING") && payoutId) return <div className="space-y-2">
    ...
    <p className="text-[11px] text-slate-500">No retry while the transfer is unresolved.</p>
    {checkState.message ? <p role="status" className={`text-xs ${checkState.ok ? "text-lime-200" : "text-rose-200"}`}>{checkState.message}</p> : null}
  </div>;

  if (status === "FAILED") return <div className="space-y-2">
    <form action={retry}><input type="hidden" name="withdrawalId" value={withdrawalId} /><input type="hidden" name="paymentType" value={defaultType} /><button type="submit" disabled={retrying} className="w-full rounded-lg border border-amber-300/20 px-3 py-2 text-xs font-bold text-amber-200 disabled:opacity-50">{retrying ? "Retrying…" : "Retry failed payout"}</button></form>
