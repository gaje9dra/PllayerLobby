...
export function WithdrawalActions({ withdrawalId, status }: { withdrawalId: string; status: string }) {
  ...
  if (status === "PENDING") return <div className="space-y-3">
    <form action={approve} onSubmit={(event) => { if (!window.confirm("Approve this withdrawal request? This does not initiate a payout.")) event.preventDefault(); }}><input type="hidden" name="withdrawalId" value={withdrawalId} /><button type="submit" disabled={approving || rejecting} className="w-full rounded-lg bg-lime-300 px-3 py-2 text-xs font-black text-slate-950 disabled:opacity-50">{approving ? "Approving…" : "Approve"}</button></form>
  ...
  if (status === "APPROVED") return <p className="text-xs font-semibold text-amber-200">Approved — payout processing is not part of Phase 5.4.</p>;
  if (status === "REJECTED" || status === "CANCELLED") return <span className="text-xs font-semibold text-slate-500">No action</span>;
