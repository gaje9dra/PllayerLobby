...
type SettlementActionState = { ok: boolean; message: string; code?: string; balance?: string; participant?: string; amount?: string; currency?: string; creditedAt?: string };
...
export function SettlementRowActions({ tournamentId, settlementId, status, participant, amount, currency }: { tournamentId: string; settlementId: string; status: "PENDING" | "APPROVED" | "CREDITED" | "CANCELLED"; participant: string; amount: string; currency: string }) {
  ...
  return <div className="flex flex-wrap items-center gap-2">
    {status === "PENDING" ? <><form action={approveFormAction}><input type="hidden" name="tournamentId" value={tournamentId} /><input type="hidden" name="settlementId" value={settlementId} /><button disabled={approvePending} className="rounded-lg bg-lime-300 px-3 py-2 text-xs font-bold text-black disabled:opacity-40">{approvePending ? "Approving…" : "Approve"}</button></form><form action={cancelFormAction} onSubmit={(event) => { if (!window.confirm("Cancel this pending settlement? The record will remain in history and no wallet credit will occur.")) event.preventDefault(); }}><input type="hidden" name="tournamentId" value={tournamentId} /><input type="hidden" name="settlementId" value={settlementId} /><button disabled={cancelPending} className="rounded-lg border border-rose-300/30 px-3 py-2 text-xs font-bold text-rose-200 disabled:opacity-40">{cancelPending ? "Cancelling…" : "Cancel"}</button></form></> : null}
    ...
    {creditState.message ? <span className={`max-w-xl text-xs ${creditState.ok ? "text-lime-200" : "text-rose-300"}`}>{creditState.message}{creditState.balance ? ` · Balance after credit: ₹${creditState.balance}` : ""}{creditState.participant ? ` · Participant: ${creditState.participant}` : ""}{creditState.amount ? ` · Amount: ₹${creditState.amount} ${creditState.currency || ""}` : ""}{creditState.creditedAt ? ` · ${new Date(creditState.creditedAt).toLocaleString()}` : ""}</span> : null}
  </div>;
}
