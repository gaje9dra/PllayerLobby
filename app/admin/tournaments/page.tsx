...
function pageUrl(params: Record<string, string | number | undefined>) { const search = new URLSearchParams(); for (const [key, value] of Object.entries(params)) if (value !== undefined && value !== "") search.set(key, String(value)); return `/admin/tournaments?${search.toString()}`; }
...
export default async function AdminTournamentsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const games = await prisma.game.findMany({ where: { isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true } });
  const where = { ...(query ? { OR: [{ name: { contains: query, mode: "insensitive" as const } }, { slug: { contains: query, mode: "insensitive" as const } }] } : {}), ...(gameId ? { gameId } : {}), ...(status ? { status } : {}) };
  const total = await prisma.tournament.count({ where });
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const page = Math.min(currentPage, totalPages);
  const tournaments = await prisma.tournament.findMany({ where, orderBy: SORT_OPTIONS[sort], skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE, select: { id: true, name: true, slug: true, startTime: true, registrationStartTime: true, registrationEndTime: true, entryFee: true, prizePool: true, maxParticipants: true, status: true, tournamentFormat: true, region: true, createdAt: true, updatedAt: true, game: { select: { name: true, logoUrl: true } } } });
  const now = new Date();
  const baseParams = { q: query, gameId, status, sort };
  return (
    <SectionContainer className="py-10 sm:py-14">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-lime-300">Administration</p><h1 className="mt-2 text-3xl font-black tracking-tight text-white sm:text-4xl">Tournaments</h1><p className="mt-2 text-sm text-slate-400">Manage tournament configuration, status, and cancellation safely.</p></div><Button href="/admin/tournaments/create">Create Tournament</Button></div>
      <form method="get" className="mt-8 rounded-2xl border border-white/10 bg-white/[0.025] p-4 sm:p-5"><div className="grid gap-3 lg:grid-cols-[2fr_1fr_1fr_1fr_auto]"><input name="q" defaultValue={query} placeholder="Search by tournament name or slug" className="min-h-11 rounded-xl border border-white/10 bg-slate-950/70 px-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-lime-300/60" /><select name="gameId" defaultValue={gameId} className="min-h-11 rounded-xl border border-white/10 bg-slate-950/70 px-4 text-sm text-white outline-none focus:border-lime-300/60"><option value="">All active games</option>{games.map((game) => <option key={game.id} value={game.id}>{game.name}</option>)}</select><select name="status" defaultValue={status ?? ""} className="min-h-11 rounded-xl border border-white/10 bg-slate-950/70 px-4 text-sm text-white outline-none focus:border-lime-300/60"><option value="">All statuses</option>{Object.values(TournamentStatus).map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select><select name="sort" defaultValue={sort} className="min-h-11 rounded-xl border border-white/10 bg-slate-950/70 px-4 text-sm text-white outline-none focus:border-lime-300/60"><option value="newest">Newest created</option><option value="oldest">Oldest created</option><option value="start_asc">Start date ascending</option><option value="start_desc">Start date descending</option></select><Button type="submit">Search</Button></div></form>
