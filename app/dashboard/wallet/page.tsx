...
export default async function WalletPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <SectionContainer className="py-10 sm:py-14">
    ...
    <div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-col items-stretch gap-3 sm:items-end"><div className="rounded-2xl border border-lime-300/20 bg-lime-300/10 px-6 py-4 text-right"><p className="text-xs font-bold uppercase tracking-wide text-lime-200">Wallet balance</p><p className="mt-1 text-3xl font-black text-white">₹{wallet.balance.toFixed(2)}</p><p className="text-xs font-semibold text-slate-400">{wallet.currency}</p></div><div className="flex gap-2"><Link href="/dashboard/wallet/payout-methods" className="rounded-xl border border-white/10 px-4 py-2 text-center text-sm font-bold text-slate-200 hover:border-lime-300/30 hover:text-lime-200">Payout methods</Link><Link href="/dashboard/wallet/withdraw" className="rounded-xl border border-white/10 px-4 py-2 text-center text-sm font-bold text-slate-200 hover:border-lime-300/30 hover:text-lime-200">Withdraw funds</Link></div></div>
    </div>
