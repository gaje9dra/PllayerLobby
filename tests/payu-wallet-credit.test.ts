...
test("verified deposit credit is idempotent at the wallet ledger boundary", async () => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await creditVerifiedDepositInTransaction(tx, { walletId: wallet.id, amount: "500.00", currency: "INR", depositReference: reference });
    });
    await prisma.$transaction(async (tx) => {
      await creditVerifiedDepositInTransaction(tx, { walletId: wallet.id, amount: "500.00", currency: "INR", depositReference: reference });
    });
    ...
    const entries = await prisma.walletTransaction.count({ where: { walletId: wallet.id, referenceType: "DEPOSIT", referenceId: reference, type: "CREDIT", category: "DEPOSIT" } });
