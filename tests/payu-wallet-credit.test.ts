...
test("verified deposit credit is idempotent at the wallet ledger boundary", async () => {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await creditVerifiedDepositInTransaction(tx, { walletId: wallet.id, amount: "500.00", currency: "INR", depositId: deposit.id });
    });
    await prisma.$transaction(async (tx) => {
      await creditVerifiedDepositInTransaction(tx, { walletId: wallet.id, amount: "500.00", currency: "INR", depositId: deposit.id });
    });
    ...
    const entries = await prisma.walletTransaction.count({ where: { walletId: wallet.id, referenceType: "DEPOSIT", referenceId: deposit.id, type: "CREDIT", category: "DEPOSIT" } });
