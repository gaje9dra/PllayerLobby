...
function systemAuditActorId() { const actorId = process.env.CRON_ACTOR_USER_ID?.trim(); if (!actorId) throw new Error("CRON_ACTOR_USER_ID is required for system lifecycle auditing."); return actorId; }
...
async function transitionTournament(tournament: { id: string; status: TournamentStatus }, next: TournamentStatus, actorUserId: string, now: Date) {
  return prisma.$transaction(async (tx) => {
    const result = await tx.tournament.updateMany({ where: { id: tournament.id, status: tournament.status }, data: { status: next } });
    if (result.count !== 1) return false;
    await recordAdminAuditEventInTransaction(tx, actorUserId, { action: "TOURNAMENT_LIFECYCLE_TRANSITIONED", targetType: "TOURNAMENT", targetId: tournament.id, metadata: { actor: "SYSTEM", fromStatus: tournament.status, toStatus: next, timestamp: now.toISOString() } });
    return true;
  });
}
...
export async function refreshTournamentLifecycle(tournamentId: string) {
  ...
  const actorUserId = systemAuditActorId();
  let status = tournament.status;
  for (let i = 0; i < 3; i += 1) {
    const next = getDueTournamentStatus(lifecycleInput({ ...tournament, status }), now);
    if (!next) break;
    const changed = await transitionTournament({ id: tournament.id, status }, next, actorUserId, now);
    if (!changed) { const current = await prisma.tournament.findUnique({ where: { id: tournament.id }, select: { status: true } }); return current?.status ?? null; }
    status = next;
  }
  return status;
}
...
export async function updateDueTournamentLifecycles() {
  ...
  for (let pass = 0; pass < 3; pass += 1) {
    ...
    for (const tournament of candidates) {
      const next = getDueTournamentStatus(lifecycleInput(tournament), now); if (!next) continue;
      if (await transitionTournament({ id: tournament.id, status: tournament.status }, next, actorUserId, now)) updated += 1;
    }
  }
