...
function getEncryptionKey() {
  ...
  return createHash("sha256").update(`${secret}:tournament-room`, "utf8").digest();
}
...
function decryptSecret(payload: string) {
  ...
  if (!iv || !tag || !ciphertext) throw new Error("Invalid encrypted room credential.");
  const decipher = createDecipheriv("aes-256-gcm", getEncryptionKey(), Buffer.from(iv, "base64url"));
  decipher.setAuthTag(Buffer.from(tag, "base64url"));
  return Buffer.concat([decipher.update(Buffer.from(ciphertext, "base64url")), decipher.final()]).toString("utf8");
}

function normalizeRoomValue(value: string, maxLength: number) {
...
export async function upsertTournamentRoom(input: {
  tournamentId: string;
  roomId: string;
  roomPassword: string;
  published: boolean;
  ...
  if (tournament.status === TournamentStatus.COMPLETED || tournament.status === TournamentStatus.CANCELLED) {
    return { ok: false, message: "Room credentials cannot be activated for a completed or cancelled tournament." };
  }
  ...
  await prisma.tournamentRoom.upsert({
    ...
    create: {
      ...
      roomIdEncrypted: encryptSecret(roomId),
      roomPasswordEncrypted: encryptSecret(roomPassword),
    ...
    update: {
      ...
      roomPasswordEncrypted: encryptSecret(roomPassword),
      publishedAt: input.published ? now : null,
      revokedAt: input.published ? null : new Date(),
    },
  });
  return { ok: true };
}
...
export type ParticipantRoomAccess =
  | { ok: true; roomId: string; roomPassword: string }
...
export async function getParticipantRoomAccess(input: { tournamentId: string; registrationId: string; registrationCode: string }): Promise<ParticipantRoomAccess> {
  ...
  const registration = await prisma.registration.findUnique({
    ...
    select: { id: true, userId: true, tournamentId: true, status: true },
  });
  ...
  const { validateRegistrationCode } = await import("@/lib/registration-code");
  ...
  if (!isJoiningWindowOpen(new Date(), tournament.startTime, tournament.joiningWindowMinutes)) {
    return { ok: false, reason: "Joining information will become available shortly before the tournament." };
  }
