...
async function applyVerifiedWalletDeposit(verification: PayUVerificationResult, merchantTransactionId: string, response: PayUResponseFields): Promise<WalletDepositVerificationResult> {
  return prisma.$transaction(async (tx) => {
    ...
    if (verification.state === "SUCCESS") {
      ...
      await creditVerifiedDepositInTransaction(tx, { walletId: deposit.walletId, amount: deposit.amount.toFixed(2), currency: deposit.currency, depositId: deposit.id, description: `Wallet deposit ${deposit.reference}` });
