...
export const ALLOWED_TOURNAMENT_STATUS_TRANSITIONS: Record<
  TournamentStatus,
...
export function getDueTournamentStatus(
  tournament: TournamentLifecycleInput,
  now: Date,
): TournamentStatus | null {
  if (tournament.status === TournamentStatus.DRAFT || tournament.status === TournamentStatus.CANCELLED || tournament.status === TournamentStatus.COMPLETED) {
    return null;
  }
...
export function isStatusTransitionDue(
  tournament: TournamentLifecycleInput,
  now: Date,
) {
...
export function canAdminSetTournamentStatus(
