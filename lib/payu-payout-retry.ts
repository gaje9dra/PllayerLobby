...
export async function retryFailedWithdrawalPayout(withdrawalId: string, paymentType?: string) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (payout?.status !== PayoutStatus.FAILED) throw new Error("PAYOUT_RETRY_NOT_ALLOWED");
    if (request.status !== WithdrawalRequestStatus.FAILED) throw new Error("PAYOUT_RETRY_NOT_ALLOWED");
