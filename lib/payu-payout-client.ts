...
function baseUrl(environment: PayUPayoutEnvironment) {
  return environment === "PRODUCTION" ? "https://payout.payumoney.com" : "https://uatoneapi.payu.in";
}

function authUrl(environment: PayUPayoutEnvironment) {
  return environment === "PRODUCTION" ? "https://accounts.payu.in/oauth/token" : "https://uat-accounts.payu.in/oauth/token";
}
...
async function requestJson(url: string, init: RequestInit) {
  ...
  if (!response.ok) {
    const safe = body && typeof body === "object" && "msg" in body ? String((body as { msg?: unknown }).msg ?? "Provider request failed").slice(0, 300) : "Provider request failed";
...
async function fetchAccessToken(forceRefresh = false) {
  ...
  tokenPromise = (async () => {
    ...
    const response = await requestJson(authUrl(config.environment), { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded", "cache-control": "no-cache" }, body });
    const accessToken = typeof response === "object" && response && "access_token" in response ? String((response as { access_token: unknown }).access_token) : "";
    const expiresIn = typeof response === "object" && response && "expires_in" in response ? Number((response as { expires_in: unknown }).expires_in) : 0;
...
function authHeaders(token: string, merchantId: string) {
  return { Authorization: `Bearer ${token}`, payoutMerchantId: merchantId, "Content-Type": "application/json" };
}

export type PayUBeneficiaryInput = { name: string; email?: string; mobile?: string; accountNo?: string; ifsc?: string; vpa?: string };

export async function createPayUBeneficiary(input: PayUBeneficiaryInput) {
  return withAuth(async (token, config) => requestJson(`${baseUrl(config.environment)}/payout/beneficiary`, { method: "POST", headers: authHeaders(token, config.merchantId), body: JSON.stringify(input) }));
}
