...
import { RegistrationStatus, TournamentPrizeSettlementStatus, TournamentResultStatus } from "@/app/generated/prisma/client";
...
export async function createTournamentResult(tournamentId: string, registrationId: string, rankInput: string, scoreInput: string): Promise<ResultOperation> {
  await requireAdmin(); const rank = parsePositiveRank(rankInput); const score = parseScore(scoreInput);
  if (rank === null) return { ok: false, message: "Rank must be a positive integer." }; if (score === null) return { ok: false, message: "Score must be a non-negative number with at most 6 decimal places." };
  ...
  try { const result = await prisma.tournamentResult.create({ data: { tournamentId, registrationId, rank, score, resultStatus: TournamentResultStatus.DRAFT }, select: { id: true } }); return { ok: true, resultId: result.id }; }
  catch (error) { if (isUniqueError(error)) return { ok: false, message: "A result already exists for this registration." }; console.error("Result creation failed:", error); return { ok: false, message: "Unable to create the result." }; }
}
...
export async function updateTournamentResult(tournamentId: string, resultId: string, rankInput: string, scoreInput: string): Promise<ResultOperation> {
  await requireAdmin(); const rank = parsePositiveRank(rankInput); const score = parseScore(scoreInput);
  if (rank === null) return { ok: false, message: "Rank must be a positive integer." }; if (score === null) return { ok: false, message: "Score must be a non-negative number with at most 6 decimal places." }; if (!UUID.test(tournamentId) || !UUID.test(resultId)) return { ok: false, message: "Invalid result identifier." };
  ...
  try { await prisma.tournamentResult.update({ where: { id: result.id }, data: { rank, score } }); return { ok: true, resultId: result.id }; }
  catch (error) { console.error("Result update failed:", error); return { ok: false, message: "Unable to update the result." }; }
}
...
async function transitionResult(tournamentId: string, resultId: string, target: TournamentResultStatus): Promise<ResultOperation> {
  ...
  try { await prisma.tournamentResult.update({ where: { id: result.id }, data: { resultStatus: target } }); return { ok: true, resultId: result.id }; }
  catch (error) { if (isUniqueError(error) && target === TournamentResultStatus.VERIFIED) return { ok: false, message: "Another verified result claimed this rank first. Refresh and review the rankings." }; console.error("Result status update failed:", error); return { ok: false, message: "Unable to change the result status." }; }
}
