...
import { getNetlifyAviatorRoundSnapshot } from "./serverless";
...
export const SERVERLESS_AVIATOR_CODES = {
...
type Code = typeof SERVERLESS_AVIATOR_CODES[keyof typeof SERVERLESS_AVIATOR_CODES];
type Result<T> = ({ ok: true } & T) | { ok: false; code: Code; message: string };
...
export async function placeNetlifyAviatorBetForUser(
  ...
  if (!UUID.test(input.roundId) || !input.clientRequestId || input.clientRequestId.length > 128) return { ok: false, code: SERVERLESS_AVIATOR_CODES.INVALID_BET_AMOUNT, message: message(SERVERLESS_AVIATOR_CODES.INVALID_BET_AMOUNT) };
  const amount = validateAmount(input.amount);
  if (!amount) return { ok: false, code: SERVERLESS_AVIATOR_CODES.INVALID_BET_AMOUNT, message: message(SERVERLESS_AVIATOR_CODES.INVALID_BET_AMOUNT) };
  const auto = validateAuto(input.autoCashoutMultiplier);
  if (auto === "invalid") return { ok: false, code: SERVERLESS_AVIATOR_CODES.INVALID_AUTO_CASHOUT, message: message(SERVERLESS_AVIATOR_CODES.INVALID_AUTO_CASHOUT) };

  const rate = await consumeSecurityRateLimit({ namespace: "aviator-bet", key: user.id, limit: 20, windowSeconds: 10 });
  if (!rate.allowed) return { ok: false, code: SERVERLESS_AVIATOR_CODES.RATE_LIMITED, message: message(SERVERLESS_AVIATOR_CODES.RATE_LIMITED) };

  const snapshot = await getNetlifyAviatorRoundSnapshot();
  if (snapshot.roundId !== input.roundId || snapshot.phase !== "WAITING") {
    const code = snapshot.phase === "RUNNING" ? SERVERLESS_AVIATOR_CODES.ROUND_ALREADY_RUNNING : snapshot.phase === "CRASHED" ? SERVERLESS_AVIATOR_CODES.ROUND_ALREADY_CRASHED : SERVERLESS_AVIATOR_CODES.ROUND_NOT_ACCEPTING_BETS;
    return { ok: false, code, message: message(code) };
  }

  try {
    const result = await prisma.$transaction(async (tx) => {
      const locked = await tx.$queryRaw<Array<{ id: string; status: string }>>(Prisma.sql`SELECT "id", "status" FROM "AviatorRound" WHERE "id" = ${snapshot.roundId}::uuid FOR UPDATE`);
      if (!locked[0] || locked[0].status !== "WAITING") throw new Error("ROUND_NOT_ACCEPTING_BETS");
    ...
    if (error instanceof Error && error.message === "INSUFFICIENT_BALANCE") return { ok: false, code: SERVERLESS_AVIATOR_CODES.INSUFFICIENT_BALANCE, message: message(SERVERLESS_AVIATOR_CODES.INSUFFICIENT_BALANCE) };
    if (error instanceof Error && error.message === "DUPLICATE_REQUEST") return { ok: false, code: SERVERLESS_AVIATOR_CODES.DUPLICATE_REQUEST, message: message(SERVERLESS_AVIATOR_CODES.DUPLICATE_REQUEST) };
    if (error instanceof Error && error.message === "ROUND_NOT_ACCEPTING_BETS") return { ok: false, code: SERVERLESS_AVIATOR_CODES.ROUND_ALREADY_RUNNING, message: message(SERVERLESS_AVIATOR_CODES.ROUND_ALREADY_RUNNING) };
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") return { ok: false, code: SERVERLESS_AVIATOR_CODES.DUPLICATE_REQUEST, message: message(SERVERLESS_AVIATOR_CODES.DUPLICATE_REQUEST) };
    console.error("Netlify Aviator bet placement failed", error);
    return { ok: false, code: SERVERLESS_AVIATOR_CODES.WALLET_TRANSACTION_FAILED, message: message(SERVERLESS_AVIATOR_CODES.WALLET_TRANSACTION_FAILED) };
  }
}
...
export async function cashoutNetlifyAviatorBetForUser(user: Pick<CurrentUser, "id" | "status">, betId: string, roundId: string): Promise<Result<{ betId: string; roundId: string; multiplier: number; payout: string; status: string }>> {
  ...
  if (snapshot.roundId !== roundId || snapshot.phase !== "RUNNING") {
    const code = snapshot.phase === "CRASHED" ? SERVERLESS_AVIATOR_CODES.ROUND_ALREADY_CRASHED : SERVERLESS_AVIATOR_CODES.CASHOUT_TOO_LATE;
    return { ok: false, code, message: message(code) };
  }
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (lockedRound[0].status !== "RUNNING") return { kind: "late" as const };
      const bet = await tx.aviatorBet.findUnique({ where: { id: betId } });
      if (!bet || bet.userId !== user.id || bet.roundId !== roundId) return { kind: "not-found" as const };
      if (bet.status !== "ACTIVE") return { kind: "settled" as const };
      const payout = multiplyMoneyByMultiplier(bet.stake.toString(), snapshot.multiplier.toFixed(2));
      const updated = await tx.aviatorBet.updateMany({ where: { id: betId, status: "ACTIVE" }, data: { status: "CASHED_OUT", cashedOutAt: new Date(snapshot.serverTime), cashoutMultiplier: snapshot.multiplier, payout } });
      ...
      return { kind: "success" as const, payout, multiplier: snapshot.multiplier };
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    ...
    return { ok: false, code: SERVERLESS_AVIATOR_CODES.BET_NOT_FOUND, message: message(SERVERLESS_AVIATOR_CODES.BET_NOT_FOUND) };
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2034") return { ok: false, code: SERVERLESS_AVIATOR_CODES.BET_ALREADY_SETTLED, message: message(SERVERLESS_AVIATOR_CODES.BET_ALREADY_SETTLED) };
    console.error("Netlify Aviator cashout failed", error);
    return { ok: false, code: SERVERLESS_AVIATOR_CODES.WALLET_TRANSACTION_FAILED, message: message(SERVERLESS_AVIATOR_CODES.WALLET_TRANSACTION_FAILED) };
  }
}
