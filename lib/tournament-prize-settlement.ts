...
export async function approvePrizeSettlement(tournamentId: string, settlementId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (settlement.prize.status !== TournamentPrizeStatus.FINALIZED) throw new Error("The prize configuration is not finalized.");
    if (settlement.registration.status !== RegistrationStatus.CONFIRMED || settlement.registration.tournamentId !== tournamentId) throw new Error("The winner registration is no longer eligible.");
    if (settlement.result.resultStatus !== TournamentResultStatus.VERIFIED || settlement.result.tournamentId !== tournamentId || settlement.result.registrationId !== settlement.registrationId || settlement.result.rank !== settlement.rank) throw new Error("The verified result is no longer valid for this settlement.");
    if (settlement.prize.rank !== settlement.rank || settlement.prize.status !== TournamentPrizeStatus.FINALIZED) throw new Error("The prize position no longer matches the settlement.");
    if (settlement.currency !== SETTLEMENT_CURRENCY || compareMoney(settlement.amount.toString(), settlement.prize.amount.toString()) !== 0) throw new Error("Settlement amount does not match the authoritative prize snapshot.");
    return tx.tournamentPrizeSettlement.update({ where: { id: settlement.id }, data: { status: TournamentPrizeSettlementStatus.APPROVED }, select: { id: true, status: true } });
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
}

export async function cancelPrizeSettlement(tournamentId: string, settlementId: string) {
  await requireAdmin(); assertUuid(tournamentId, "tournament identifier"); assertUuid(settlementId, "settlement identifier");
  const settlement = await prisma.tournamentPrizeSettlement.findUnique({ where: { id: settlementId }, select: { id: true, tournamentId: true, status: true } });
  if (!settlement || settlement.tournamentId !== tournamentId) throw new Error("Settlement does not belong to this tournament.");
  if (!canCancelSettlement(settlement.status)) throw new Error("Only pending settlements can be cancelled.");
  return prisma.tournamentPrizeSettlement.update({ where: { id: settlement.id }, data: { status: TournamentPrizeSettlementStatus.CANCELLED }, select: { id: true, status: true } });
}
